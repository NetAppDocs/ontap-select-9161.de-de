...
.Hardware-RAID
--
Wenn ein lokaler Hardware-RAID-Controller verwendet wird, muss mindestens ein Datenspeicher (ESX) oder ein link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["Speicherpool (KVM)"] auf jedem Knoten für die Systemdaten sowie die Root- und Datenaggregate erstellt werden. Der Speicherpool ist im Rahmen der Konfiguration des ONTAP Select Knotens anzubinden.

--
.Software-RAID
--
Bei Verwendung von Software-RAID ist mindestens ein Datenspeicher (ESX) oder ein link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["Speicherpool (KVM)"] für die Systemdaten zu erstellen, und es ist sicherzustellen, dass die SSD-Laufwerke für das Root- und die Datenaggregate verfügbar sind. Der Speicherpool und die Festplatten sind im Rahmen der Konfiguration des ONTAP Select Knotens anzubinden.

--
====
. Verfügbare ONTAP Select -Versionen
+
Das Deploy-Verwaltungsprogramm enthält eine einzige Version von ONTAP Select. Wenn Sie Cluster mit einer früheren Version von ONTAP Select bereitstellen möchten, müssen Sie zunächst link:task_cli_deploy_image_add.html["Fügen Sie das ONTAP Select Image hinzu"] zu Ihrer Deploy-Instanz.

. Lizenzieren Sie ONTAP Select für eine Produktionsbereitstellung
+
Bevor Sie einen ONTAP Select Cluster in einer Produktionsumgebung einsetzen, müssen Sie eine Speicherkapazitätslizenz erwerben und die zugehörige Lizenzdatei herunterladen. Sie können link:concept_lic_production.html["Lizenzieren Sie den Speicher an jedem Knoten"] mithilfe des _Capacity Tiers_-Modells oder lizenzieren Sie einen gemeinsam genutzten Pool mithilfe des _Capacity Pools_-Modells.
...
.Bevor Sie beginnen
Führen Sie den Netzwerkkonnektivitätsprüfer mit dem link:task_adm_connectivity.html["Web-Benutzeroberfläche"] oder die link:task_cli_connectivity.html["Befehlszeilenschnittstelle (CLI)"] um die Konnektivität zwischen den Clusterknoten im internen Netzwerk zu bestätigen.
...
.Nach Abschluss
Sie sollten die ONTAP Select Deploy-Konfigurationsdaten sichern.
