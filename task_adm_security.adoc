...
|===
. Nachdem der neue Management-Server hinzugefügt wurde, können Sie optional auf image:icon_kebab.gif["Optionen"] und wählen Sie eine der folgenden Optionen aus:
+
...
Konfigurieren Sie die YubiKey-PIN und generieren oder importieren Sie den privaten Schlüssel und das Zertifikat des Remote Support Agent (RSA) oder Elliptic Curve Digital Signature Algorithm (ECDSA) mit den Schritten in link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647: Multifaktor-Authentifizierung in ONTAP"^] .
...
Wenn Sie sich für die YubiKey FIDO2-Authentifizierung entscheiden, konfigurieren Sie die YubiKey FIDO2-PIN mit dem YubiKey Manager und generieren Sie den FIDO2-Schlüssel mit einer PuTTY-CAC (Common Access Card) für Windows oder einem ssh-keygen für MacOS. Die Schritte dazu finden Sie im technischen Bericht. link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647: Multifaktor-Authentifizierung in ONTAP"^] .
...
.Für MacOS:
* Der öffentliche PIV-Schlüssel sollte mit dem  `ssh-keygen -e` Befehl wie im Abschnitt *Konfigurieren des Mac OS- oder Linux-SSH-Clients für die YubiKey-PIV-Authentifizierung* auf Seite 24 von TR-4647 beschrieben.
* Der öffentliche FIDO2-Schlüssel befindet sich im  `id_ecdsa_sk.pub` Datei oder  `id_edd519_sk.pub` Datei, je nachdem, ob Sie ECDSA oder EDD519 verwenden, wie im Abschnitt *Konfigurieren des MAC OS- oder Linux-SSH-Clients für die YubiKey FIDO2-Authentifizierung* auf Seite 39 von TR-4647 beschrieben.
...
.Bevor Sie beginnen
Legen Sie den öffentlichen PIV- oder FIDO2-Schlüssel fest, der für den YubiKey konfiguriert ist. Der ONTAP Select Deploy CLI-Befehl  `security publickey add -key` ist für PIV oder FIDO2 gleich und die öffentliche Schlüsselzeichenfolge ist unterschiedlich.
...
* Exportieren des öffentlichen Schlüssels in einem SSH-kompatiblen Format mithilfe der  `ssh-keygen -e` Befehl für PIV
* Die öffentliche Schlüsseldatei befindet sich im  `~/.ssh/id_***_sk.pub` Datei für FIDO2 (MacOS)


.Schritte
. Den generierten Schlüssel finden Sie im  `.ssh/id_***.pub` Datei.
. Fügen Sie den generierten Schlüssel zu ONTAP Select Deploy hinzu, indem Sie  `security publickey add -key <key>` Befehl.
+
[listing]
----
...
Der  `ssh-keygen` Der Befehl ist ein Tool zum Erstellen neuer Authentifizierungsschlüsselpaare für SSH. Die Schlüsselpaare werden zur Automatisierung von Anmeldungen, Single Sign-On und zur Authentifizierung von Hosts verwendet.

Der  `ssh-keygen` Der Befehl unterstützt mehrere Public-Key-Algorithmen für Authentifizierungsschlüssel.

* Die Auswahl des Algorithmus erfolgt mit dem  `-t` Option
* Die Auswahl der Schlüsselgröße erfolgt mit dem  `-b` Option
...
* Wenn Sie sich als Administrator über Secure Shell (SSH) bei der Deploy-CLI anmelden können, deaktivieren Sie MFA, indem Sie den  `security multifactor authentication disable` Befehl von der Bereitstellungs-CLI.
+
[listing]
----
...
.. Führen Sie den  `security multifactor authentication disable` Befehl.
+
[listing]
----
