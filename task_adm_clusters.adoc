...
. Klicken image:icon_kebab.gif["Optionen"] rechts neben dem Cluster und wählen Sie *Offline schalten*.
+
...
. Um den Cluster wieder online zu bringen, klicken Sie auf image:icon_kebab.gif["Optionen"] und wählen Sie *Online schalten*.
...
. Sign in mit dem Administratorkonto bei der Web-Benutzeroberfläche des Bereitstellungsdienstprogramms an.
. Klicken Sie oben auf der Seite auf die Registerkarte *Cluster* und wählen Sie den gewünschten Cluster aus der Liste aus.
. Klicken image:icon_kebab.gif["Optionen"] rechts neben dem Cluster und wählen Sie *Löschen*.
+
...
NOTE: Anstatt die Bereitstellungs-GUI zu verwenden, können Sie den Befehl „Cluster aktualisieren“ in der Bereitstellungs-CLI-Shell verwenden, um einen Cluster zu aktualisieren.
...
. Klicken image:icon_kebab.gif["Optionen"] auf der rechten Seite der Seite und wählen Sie *Cluster aktualisieren*.
